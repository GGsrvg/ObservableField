...
extension ObservableDataSource {
    ...
    open func addRows(_ rows: [SI.Row], to section: SI) {
        guard let sectionIndex = self.findSectionIndex(section) else { return }
        
        self.addRows(rows, by: sectionIndex)
    }
    
    open func addRows(_ rows: [SI.Row], inFirst predicate: (SI) throws -> Bool) {
    ...
    open func updateRow(_ row: SI.Row) {
        self.replaceRow(row, oldRow: row)
    }
    
    open func removeRow(_ row: SI.Row) {
